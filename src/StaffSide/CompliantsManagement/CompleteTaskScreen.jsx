...
const CompleteTaskScreen = ({ route }) => {
  ...
  const complains = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_URL}/complain/eachComplainToComplete`,
        {
          complainId
        },
        {
  ...
  /////////// In chance location of the person is needed ////////
  ...
  //           `${BASE_URL}/complain/completeTask`,
  //           formData,
  ...
  const completeTask = async ()=>{
    ...
    try {
      const formData = new FormData();
      formData.append('assignId', complainId)
      formData.append('remarks', remarks)
      if (photo) {
        console.log("photo:", photo);
        formData.append("photo", {
