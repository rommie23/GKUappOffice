...
const ReappearFormPhd = () => {
    ...
    const [valuesCheckModal, setValuesCheckModal] = useState('')
    ...
    const getExamFormData = async () => {
        ...
        if (session != null) {
            try {
                const examForm = await fetch(BASE_URL + '/student/examdate/6', {
                    method: 'POST',
                    headers: {
                ...
                console.log('data froms api examForm phd reappear:::', examFormDetails['statusopen'][0])
                setLoading(false)
                // console.log(transactions);
            } catch (error) {
                console.log('Error fetching form data:examFrom phd reappear:', error)
                setLoading(false)
                errorModel(ALERT_TYPE.WARNING, "Oops!!!", `Something went wrong !!`)
                // setShowModal2(true)
            }
    ...
    const getSubjects = async () => {
        ...
        if (session != null) {
            try {
                ...
                // console.log('data froms api examForm Subjects phd reappear:::',examSemesterDetails['data'])
                setLoading(false)
            } catch (error) {
                console.log('Error fetching form data:examFrom reappear:', error)
                setLoading(false)
                errorModel(ALERT_TYPE.WARNING, "Oops!!!", `Something went wrong !!`)
                // setShowModal2(true)
            }
    ...
    const onSubmit = async () => {
        if (selectedCourseMedium == 0) {
            ...
            setStudentDataArray(studentDataArray.push({ "SemId": selectedSemester, "Examination": examFormData["examination"], "Type": "Reappear", "Group": "NA", "moi": selectedCourseMedium }))
            ...
            if (session != null) {
                ...
                try {
                    ...
                    if (examFormSubmitDataDetails["flag"] == '1') {
                        errorModel(ALERT_TYPE.WARNING, "Done", `Exam Form Alredy Submitted.`)
...
export default ReappearFormPhd
