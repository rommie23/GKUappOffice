...
const UpdateLectureAttendance = ({ route }) => {
  const { lecture, selectedDate } = route.params;
  ...
  const studentsinLecture = async () => {
    ...
    if (session != null) {
      try {
        setIsLoading(true)
        const students = await fetch(`${BASE_URL}/staff/updateAttendanceList`, {
          method: 'POST',
          headers: {
        ...
        if (studentDetails.flag == 1) {
          setStudent(studentDetails['list']);
          setCheckboxStates(studentDetails['list'].map((attendance) => ({ attendance: attendance.Attendance == '1' ? 'present' : 'absent' })))
        }
        else {
  ...
  const submitAttendance = async (arrToSend) => {
    ...
    if (session != null) {
      try {
        setIsLoading(true)
        const dailyLectures = await fetch(BASE_URL + '/Staff/updateAttendance', {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${session}`,
            'Content-Type': "application/json"
  ...
  const submitHandle = async () => {
    ...
    for (let j = 0; j < student.length; j++) {
      ...
      arrToSend.push({ 
        IDNo: student[j]['IDNo'],
        Attendance: `${student[j]['attendance'] == 'present' ? 1 : 0}`,
        ID: student[j]['ID'] })
    }
...
export default UpdateLectureAttendance
