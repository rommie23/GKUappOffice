...
const StudentLeaveCheck = () => {
  ...
  const hostelLeaves = async () => {
    setIsLoading(true)
    const session = await EncryptedStorage.getItem("user_session")

    if (session != null) {
      try {
        const res = await axios.get(`${BASE_URL}/staff/approvedHostelLeaves`,
          {
            headers: {
        ...
        // console.log("hostelApprovedLeaves::", res.data);
  ...
  const searchStudent = async () => {
    ...
    if (session != null) {
      try {
        const res = await axios.post(`${BASE_URL}/staff/leaveSearchStudent`,
          {
            rollNo: rollNo
          },
          {
  ...
  const handleButtons = async (action, ID) => {
    ...
    if (session != null) {
      try {
        ...
        const res = await axios.post(`${BASE_URL}/staff/leaveCheckAction`, {
          action: action,
          leaveId: ID
        },
          {
  ...
  return (
    <AlertNotificationRoot>
      <View style={styles.container}>
        ...
        <Text style={styles.heading}>Student Leaves</Text>
        <ScrollView
                    ...
                    item.DepatureTime == null && item.ArrivalTime == null ?
                      ...
                      : item.DepatureTime != null && item.ArrivalTime == null ?
                        <TouchableOpacity style={styles.approveBtn}
                          onPress={() => handleButtons(2, item.ID)}>
                          <Text style={styles.buttonText}>
                            Check In Student
                          </Text>
                        </TouchableOpacity>
                        : item.DepatureTime != null && item.ArrivalTime != null ?
                          <Text style={[styles.label, { color: '#2E7D32' }]}>Status : Leave Completed</Text>
                          : null
                  }
                </View>
...
export default StudentLeaveCheck;
