...
const ChangePassword = () => {
  ...
  const { setIsLoggedin, mobileToken } = useContext(StudentContext);
  ...
  const removeSession = async () => {
    ...
    if (session != null) {
      try {
        ...
        if (response.flag == 1) {
          changePasswordSuccessModel(ALERT_TYPE.SUCCESS, "Password Changed", `Kindly Login Again with new password`)
        } else {
      ...
      if (session != null) {
        try {
          ...
          sendNewPasswordRes["Response"] == "2" ? setOldPasswordError(true) :
            sendNewPasswordRes["Response"] == "1" ? removeSession() :
              null
        } catch (error) {
  ...
  const changePasswordSuccessModel = (type, title, message) => {
    Dialog.show({
      type: type,
      title: title,
      textBody: message,
