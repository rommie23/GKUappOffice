...
const StudentMovementRequests = () => {
  ...
  const hostelMovements = async () => {
    ...
    if (session != null) {
      try {
        console.log("hostelMovements");
        const res = await axios.get(`${BASE_URL}/staff/allMovements`,
          {
            headers: {
  ...
  const handleButtons = async (action, ID) => {
    ...
    if (session != null) {
      try {
        const res = await axios.post(`${BASE_URL}/staff/movementAction`, {
          action: action,
          movementId: ID
        },
          {
  ...
  const searchStudent = async () => {
    ...
    if (session != null) {
      try {
        const res = await axios.post(`${BASE_URL}/staff/movementSearchStudentHostel`,
          {
            rollNo: rollNo
          },
          {
              ...
              {/* Buttons */}
              {
                item.Status == 'Pending' ?
                  ...
                  : item.Status == 'Accepted' ?
                    <Text style={[styles.label, { color: "#2E7D32" }]}>Status : Approved</Text>
                    : item.Status == 'Rejected' ?
                      <Text style={[styles.label, { color: colors.uniRed }]}>Status : Rejected</Text>
                      : null
              }
            </View>
...
export default StudentMovementRequests;
