...
const ReappearForm = () => {
    ...
    const getExamFormData = async () => {
        ...
        if (session != null) {
            try {
                const examForm = await fetch(BASE_URL + '/student/examdate/2', {
                    method: 'POST',
                    headers: {
                ...
                console.log('data froms api examFormReappear:::', examFormDetails['statusopen'][0])
                setLoading(false)
            } catch (error) {
    ...
    // ////////////// Subjects ARRAY (it's reappear so both come in one array [normal and elective]) /////////////////////// //
    const getElectiveSubjects = async () => {
        setLoading(true)
        const session = await EncryptedStorage.getItem("user_session")
        if (session != null) {
            try {
                const examSemester = await fetch(`${BASE_URL}/student/reappear/${selectedSemester}/${selectedGroup}`, {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${session}`
                    },
                ...
                console.log(examSemesterDetails['data']);

                if (examSemesterDetails['data'].length > 0) {
                    setShowSubmit(true)
                }

                console.log('data from api examForm Subjects reappear:::', examSemesterDetails['data'])
                setLoading(false)
            } catch (error) {
                console.log('Error fetching form data subjects :examFrom reappear:', error)
                setLoading(false)
                errorModel(ALERT_TYPE.WARNING, "Oops!!!", `Something went wrong !!`)
                // setShowModal2(true)
            }
    ...
    const searchHandler = () => {
        ...
        // console.log('group is :::',selectedGroup);
    ...
    const onSubmit = async () => {
        if (selectedCourseMedium == 0) {
            ...
            setStudentDataArray(studentDataArray.push({ "SemId": selectedSemester, "Examination": examFormData["examination"], "Type": "Reappear", "Group": selectedGroup, "moi": selectedCourseMedium }))
            ...
            const session = await EncryptedStorage.getItem("user_session")
            if (session != null) {
                console.log("examform studentDataArray data to send", studentDataArray);
                        ...
                        examFormData['formstatus'] == 0 ?
                            <ScrollView style={{ width: '100%' }}>
                                <Text style={{ flex: 1, fontSize: 24, color: '#1b1b1b', alignSelf: 'center' }}>
                                    {examFormData["examination"]} Reappear</Text>
