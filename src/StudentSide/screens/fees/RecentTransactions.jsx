...
const RecentTransactions = () => {
  ...
  const resyncApiEazyPay = async (txid) => {
    setLoading(true);
    const session = await EncryptedStorage.getItem("user_session");
    ...
    if (session != null) {
      try {
        const paymentDetailsData = await axios.post(
          BASE_URL + '/student/resyncEasyPay',
          {
            "transaction_id": txid
          },
          {
        ...
        if (parsedData.flag != 1) {
          inModel(ALERT_TYPE.INFO, "Updated", parsedData.message)
        } else {
                    ...
                    <View>
                      {
                        transaction['Gateway'] == 'razorpay' ?
                          ...
                          transaction['Gateway'] == 'eazypay' ?
                            <TouchableOpacity style={[styles.modalBtn, { borderRadius: 4, backgroundColor: colors.uniBlue }]} onPress={() => { resyncApiEazyPay(transaction['txnid']) }}>
                              <Text style={[styles.textSmall]}><Foundation size={12} name='refresh' /> ReCheck</Text>
                            </TouchableOpacity> :
