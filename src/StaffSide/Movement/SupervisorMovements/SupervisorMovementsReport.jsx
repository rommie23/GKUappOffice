...
const SupervisorMovementGranted = () => {
  ...
  const comingMovements = async () => {
    ...
    if (session != null) {
      try {
        const movementDetails = await fetch(BASE_URL + `/staff/authsidemovementreports`, {
          method: 'POST',
          headers: {
        ...
        // console.log('from api supervisorMovement Report ',movementDetailsData);
        setLoading(false);
      } catch (error) {
        console.log('Error fetching supervisorMovement Report Api ::', error);
        setLoading(false)
            ...
            <View key={i} style={styles.card}>
                ...
                <View style={styles.topCard}>
                  ...
                  <View style={{width:'30%'}}>
                    ...
                    <Text style={[styles.textStyle, styles.rowMiddle]}>{item["RequestTime"] ? item['RequestTime'].split("T")[0].split("-").reverse().join("-") : "N/A"},{item["RequestTime"] ? item['RequestTime'].substr(11,5) : 'N/A'}</Text>
                  </View>
                  <View style={{width:'30%'}}>
